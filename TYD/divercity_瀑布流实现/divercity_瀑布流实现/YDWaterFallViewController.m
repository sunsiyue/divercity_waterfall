...
@property (nonatomic, strong) NSMutableArray *cellSizesH;
@property(nonatomic, weak) UIImageView *discoverView;
...
{
    if (_discoverView == nil) {
        
        UIImageView *discoverView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.waterflowView.bounds.size.width, self.waterflowView.bounds.size.height)];
        discoverView.backgroundColor = [UIColor colorWithRed:1.0/255.0 green:1.0/255.0 blue:1.0/255.0 alpha:0.5];
        discoverView.userInteractionEnabled=YES;
        [self.waterflowView addSubview:discoverView];
...
- (UIImageView *)postImageView
{
    if (_postImageView == nil) {
        ...
        [self.discoverView addSubview:self.postImageView];
    }
...
//    [self GetRandomWithStartIndex:0 andEndIndex:self.waters.count];
//    return waterflowView.cellWidth * ([self GetRandomWithStartIndex:index andEndIndex:self.waters.count]);
//    double val = floorf(((double)arc4random() / ARC4RANDOM_MAX) * 1.10f) + 0.8 ;
//    return waterflowView.cellWidth * (arc4random() % 2 + 0.5);
//    (arc4random() % 0.9) + 1
//    return waterflowView.cellWidth * val;
    switch (index % 3) {
        case 0: return 220;
        case 1: return 250;
        case 2: return 150;
